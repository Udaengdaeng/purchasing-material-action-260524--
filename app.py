...
def status_rank(s: str) -> int:
    return {"부족": 0, "입고대기": 1, "일부입고": 2, "확보됨": 3, "수요없음": 4, "확인필요": 5}.get(s, 9)
...
def parse_material_inventory(raw: pd.DataFrame, tolerance_qty: float = 1.0) -> pd.DataFrame:
    """전체 자재 재고 화면용 파서.

    parse_purchase()는 현재 주문/생산에 필요한 자재(required > 0)만 남기므로,
    자재 마스터 관점의 전체 자재 재고를 보기에는 부족하다.
    이 함수는 구매데이터 시트의 유효 자재 행을 모두 보존한다.

    - required > 0: 기존 부족/입고대기/일부입고/확보됨 로직 적용
    - required <= 0: 자재는 존재하지만 현재 주문 소요가 없는 것으로 보고 '수요없음' 처리
    """
    cols = [
        "source_row", "supplier", "item", "item_key", "spec", "color", "unit", "unit_price",
        "moq", "leadtime", "ttl_cons", "loss_required", "stock_qty", "foc_qty",
        ...
        "etd", "eta", "remark", "secured_qty", "need_after_stock_qty", "shortage_qty",
        "arrival_gap_qty", "open_order_qty", "coverage_rate", "status", "action", "basis",
        "estimated_shortage_amount", "estimated_total_value", "status_rank", "has_demand"
    ]
    if raw is None or raw.empty or raw.shape[1] < 35:
        return pd.DataFrame(columns=cols)
    ...
    for i in range(12, len(raw)):
        ...
        unit_price = to_num(r.iloc[10] if len(r) > 10 else 0)
        ttl_cons = to_num(r.iloc[24] if len(r) > 24 else 0)
        loss_raw = to_num(r.iloc[25] if len(r) > 25 else 0)
        required = loss_raw if loss_raw > 0 else ttl_cons
        ...
        if has_demand:
            ...
            shortage = 0.0
            need_after_stock = 0.0
            arrival_gap = max(order - received, 0) if order > 0 else 0.0
            open_order = max(order - received, 0)
            coverage = np.nan
            status = "수요없음"
            action = "현재 주문/생산 소요 없음 — 재고 보유 여부만 확인"
            basis = "자재 행은 존재하지만 TTL CONS/LOSS 기준 현재 소요량이 0"
    ...
    out = pd.DataFrame(rows, columns=cols)
    if not out.empty:
        out = out.sort_values(["status_rank", "has_demand", "shortage_qty", "arrival_gap_qty"], ascending=[True, False, False, False])
    return out



def parse_material_leadtime_master(raw: pd.DataFrame) -> pd.DataFrame:
    """자재품목리스트 시트에서 리드타임 마스터를 추출한다.

    구매데이터 시트의 LEADTIME 열이 비어 있는 경우가 있어,
    자재품목리스트의 `Lead time` 열(일반적으로 G열)을 보조 기준으로 사용한다.
    같은 엑셀 템플릿에서는 구매데이터와 자재품목리스트의 자재 행 순서가 대체로 동일하므로
    source_row를 1차 키로 사용하고, item_key/color/supplier를 보조 키로 남긴다.
    """
    cols = ["source_row", "supplier", "item", "item_key", "color", "leadtime_master", "leadtime_master_days"]
    ...
    for i in range(12, len(raw)):
        ...
        # 자재품목리스트: G열(6)이 실질 Lead time, R열(17)은 템플릿/보조 열일 수 있어 fallback으로만 사용
        lt = clean(r.iloc[6] if len(r) > 6 else "")
        if not lt and len(r) > 17:
            lt = clean(r.iloc[17])
        rows.append({
            "source_row": i + 1,
            "supplier": supplier,
            ...
            "color": color,
            "leadtime_master": lt,
            "leadtime_master_days": parse_leadtime_days(lt),
        })
    return pd.DataFrame(rows, columns=cols)


def enrich_leadtime_from_master(df: pd.DataFrame, leadtime_master: pd.DataFrame) -> pd.DataFrame:
    """구매/재고 데이터의 leadtime 공백을 자재품목리스트 리드타임으로 보강한다."""
    if df is None or df.empty:
        return df
    out = df.copy()
    if "leadtime" not in out.columns:
        out["leadtime"] = ""
    out["leadtime_source"] = np.where(out["leadtime"].apply(clean).ne(""), "구매데이터", "")

    if leadtime_master is None or leadtime_master.empty:
        out["leadtime_days"] = out["leadtime"].apply(parse_leadtime_days)
        out.loc[out["leadtime_source"].eq(""), "leadtime_source"] = "데이터 없음"
        return out

    m = leadtime_master[["source_row", "leadtime_master", "leadtime_master_days"]].drop_duplicates("source_row")
    out = out.merge(m, on="source_row", how="left")

    missing = out["leadtime"].apply(clean).eq("") & out["leadtime_master"].apply(clean).ne("")
    out.loc[missing, "leadtime"] = out.loc[missing, "leadtime_master"]
    out.loc[missing, "leadtime_source"] = "자재품목리스트"

    # source_row가 어긋나는 경우를 대비해 item_key 기준 fallback도 한 번 더 수행
    if "item_key" in out.columns and "item_key" in leadtime_master.columns:
        key_master = (
            leadtime_master[leadtime_master["leadtime_master"].apply(clean).ne("")]
            .sort_values("leadtime_master_days", ascending=False)
            .drop_duplicates("item_key")[["item_key", "leadtime_master"]]
            .rename(columns={"leadtime_master": "leadtime_master_by_item"})
        )
        out = out.merge(key_master, on="item_key", how="left")
        missing2 = out["leadtime"].apply(clean).eq("") & out["leadtime_master_by_item"].apply(clean).ne("")
        out.loc[missing2, "leadtime"] = out.loc[missing2, "leadtime_master_by_item"]
        out.loc[missing2, "leadtime_source"] = "자재품목리스트(item 매칭)"
        out = out.drop(columns=["leadtime_master_by_item"])

    out["leadtime_days"] = out["leadtime"].apply(parse_leadtime_days)
    out.loc[out["leadtime_source"].eq(""), "leadtime_source"] = "데이터 없음"
    out = out.drop(columns=[c for c in ["leadtime_master", "leadtime_master_days"] if c in out.columns])
    return out
...
def compute_supply_risk(
    ...
    pcols = [
        "source_row", "leadtime", "leadtime_days", "leadtime_source", "unit_price", "loss_required", "secured_qty",
    ...
    if "leadtime_days" not in merged.columns:
        merged["leadtime_days"] = merged["leadtime"].apply(parse_leadtime_days)
    else:
        merged["leadtime_days"] = merged["leadtime_days"].fillna(merged["leadtime"].apply(parse_leadtime_days))
    if "leadtime_source" not in merged.columns:
        merged["leadtime_source"] = np.where(merged["leadtime"].apply(clean).ne(""), "구매데이터", "데이터 없음")
...
def show_intro():
    ...
    - `자재별(주문)` 화면은 현재 주문/생산 소요가 있는 자재만 보여줍니다.
    - `자재 재고(전체)` 화면은 주문 소요가 없는 자재까지 포함해 전체 자재 재고를 보여줍니다.
    - PO별 화면은 선택한 주문에 필요한 자재의 확보 상태를 확인하는 용도입니다.
    - 날짜별 화면은 전체 이벤트 건수와 날짜/자재별 상세를 함께 보여줍니다.
    """)


def show_dashboard(purchase: pd.DataFrame, production: pd.DataFrame, material_inventory: Optional[pd.DataFrame] = None):
    st.markdown('<div class="big-title">📦 Material ERP Dashboard</div>', unsafe_allow_html=True)
    ...
    all_cnt = len(material_inventory) if material_inventory is not None and not material_inventory.empty else len(purchase)
    demand_cnt = len(purchase)
    c0, c1, c2, c3, c4 = st.columns(5)
    c0.metric("전체 자재 행", f"{all_cnt:,}", help="주문 소요가 없어도 구매데이터 시트에 존재하는 유효 자재 행 전체")
    c1.metric("주문 연동 자재", f"{demand_cnt:,}", help="TTL CONS/LOSS 기준 현재 소요량이 있는 자재")
    c2.metric("🔴 부족 자재", f"{len(shortage):,}")
    c3.metric("🟣 입고대기", f"{len(waiting):,}")
    c4.metric("🟢 확보됨", f"{len(secured):,}")
    st.caption(f"부족 수량 합계: {shortage['shortage_qty'].sum():,.1f} / 입고대기량: {waiting['open_order_qty'].sum():,.1f}")
...
def show_material_view(purchase: pd.DataFrame):
    st.header("자재별 보기 (주문 연동)")
    st.caption("TTL CONS/LOSS 기준 현재 주문 소요가 있는 자재만 대상으로 부족·입고대기·일부입고를 확인합니다.")
    if purchase.empty:
...
def show_inventory_view(material_inventory: pd.DataFrame):
    st.header("자재 재고 전체")
    st.caption("주문 소요가 없는 자재도 포함한 전체 자재 재고 화면입니다. 부족 판단용 화면이 아니라 자재 존재/재고/발주 현황 확인용입니다.")
    if material_inventory.empty:
        st.warning("조회 가능한 전체 자재 데이터가 없습니다.")
        return

    statuses = ["부족", "입고대기", "일부입고", "확보됨", "수요없음", "확인필요"]
    suppliers = sorted(material_inventory["supplier"].replace("", "공급업체 확인").dropna().unique().tolist())
    f1, f2, f3, f4 = st.columns([1, 1, 1, 1.8])
    with f1:
        status_filter = st.multiselect("상태", statuses, default=statuses)
    with f2:
        demand_filter = st.selectbox("수요 구분", ["전체", "주문 소요 있음", "주문 소요 없음"])
    with f3:
        supplier_filter = st.multiselect("Supplier", suppliers)
    with f4:
        keyword = st.text_input("자재명 / Supplier / Color 검색", key="inventory_keyword")

    data = material_inventory.copy()
    if status_filter:
        data = data[data["status"].isin(status_filter)]
    if demand_filter == "주문 소요 있음":
        data = data[data["has_demand"]]
    elif demand_filter == "주문 소요 없음":
        data = data[~data["has_demand"]]
    if supplier_filter:
        data = data[data["supplier"].replace("", "공급업체 확인").isin(supplier_filter)]
    if keyword:
        k = norm(keyword)
        data = data[
            data["item"].apply(norm).str.contains(k, na=False)
            | data["supplier"].apply(norm).str.contains(k, na=False)
            | data["color"].apply(norm).str.contains(k, na=False)
        ]

    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("전체 자재", f"{len(data):,}")
    c2.metric("주문 소요 있음", f"{int(data['has_demand'].sum()):,}")
    c3.metric("주문 소요 없음", f"{int((~data['has_demand']).sum()):,}")
    c4.metric("현재 재고 합계", f"{data['stock_qty'].sum():,.1f}")
    c5.metric("발주 잔량", f"{data['open_order_qty'].sum():,.1f}")

    left, right = st.columns([0.9, 1.1])
    with left:
        cnt = data["status"].value_counts().reset_index()
        cnt.columns = ["status", "count"]
        if not cnt.empty:
            fig = px.pie(cnt, names="status", values="count", hole=0.45, color="status", color_discrete_map=COLOR_MAP)
            fig.update_traces(textposition="inside", textinfo="label+percent")
            ...
            st.plotly_chart(fig, use_container_width=True)
    with right:
        supplier_summary = data.copy()
        supplier_summary["supplier"] = supplier_summary["supplier"].replace("", "공급업체 확인")
        sup = supplier_summary.groupby("supplier", as_index=False).agg(
            자재수=("item", "count"),
            주문소요있음=("has_demand", "sum"),
            현재재고=("stock_qty", "sum"),
            발주잔량=("open_order_qty", "sum"),
        ).sort_values("자재수", ascending=False).head(12)
        fig = px.bar(sup, x="supplier", y="자재수", text="자재수")
        fig.update_layout(height=320, margin=dict(l=10, r=10, t=20, b=10), xaxis_tickangle=-30)
        st.plotly_chart(fig, use_container_width=True)

    view = data.copy()
    view["상태"] = view["status"].apply(status_label)
    view["수요구분"] = np.where(view["has_demand"], "주문 소요 있음", "주문 소요 없음")
    view["ETA"] = pd.to_datetime(view["eta"], errors="coerce").dt.strftime("%Y-%m-%d").fillna("-")
    st.dataframe(view[["상태", "수요구분", "supplier", "item", "spec", "color", "unit", "ttl_cons", "loss_required", "stock_qty", "foc_qty", "order_total_qty", "secured_qty", "shortage_qty", "received_qty", "open_order_qty", "ETA", "remark", "basis"]].rename(columns={
        "supplier":"Supplier", "item":"자재명", "spec":"Spec", "color":"Color", "unit":"단위", "ttl_cons":"TTL CONS", "loss_required":"Loss 포함 필요량", "stock_qty":"현재 재고", "foc_qty":"F.O.C", "order_total_qty":"발주 수량", "secured_qty":"확보량", "shortage_qty":"부족량", "received_qty":"입고 수량", "open_order_qty":"미입고 발주잔량", "remark":"Remark", "basis":"판단 기준"
    }), use_container_width=True, hide_index=True)
    st.download_button("전체 자재 재고 CSV 다운로드", data=data.to_csv(index=False, encoding="utf-8-sig").encode("utf-8-sig"), file_name="material_inventory_all.csv", mime="text/csv")
...
def show_risk_analysis(
    ...
    if supply["materials"].empty:
        ...
        view_cols = [
            ...
            "leadtime_source", "unit_price", "additional_cost",
        ]
        view_cols = [c for c in view_cols if c in supply["materials"].columns]
        view = supply["materials"][view_cols].rename(columns={
            "item": "자재명",
            "supplier": "공급업체",
        ...
        if supply["critical_materials"].empty:
            ...
            cv_cols = ["item", "supplier", "new_shortage_qty", "unit", "leadtime", "leadtime_days", "leadtime_source", "additional_cost"]
            cv_cols = [c for c in cv_cols if c in supply["critical_materials"].columns]
            cv = supply["critical_materials"][cv_cols].rename(columns={
                "item": "자재명",
                "supplier": "공급업체",
                "new_shortage_qty": "부족량",
                "unit": "단위",
...
def main():
    ...
    try:
        ...
        sh_material = find_sheet(sheet_names, ["자재품목리스트", "자재품목", "품목리스트"])
        ...
        raw_material = sheets.get(sh_material, pd.DataFrame())
        leadtime_master = parse_material_leadtime_master(raw_material)
        purchase = parse_purchase(raw_purchase, tolerance_qty=float(tolerance_qty))
        purchase = enrich_leadtime_from_master(purchase, leadtime_master)
        material_inventory = parse_material_inventory(raw_purchase, tolerance_qty=float(tolerance_qty))
        material_inventory = enrich_leadtime_from_master(material_inventory, leadtime_master)
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs([
        "전체 대시보드", "자재별(주문)", "자재 재고(전체)", "PO별", "날짜별", "공급업체별", "액션 리스트", "리스크 분석", "데이터 점검"
    ])
    with tab1:
        show_dashboard(purchase, production, material_inventory)
    with tab2:
        show_material_view(purchase)
    with tab3:
        show_inventory_view(material_inventory)
    with tab4:
